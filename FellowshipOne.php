<?php 
	...
	class FellowshipOne{
		...
		public $paths = array(
			...
			// added group URI links - Bethany Clark on 02/16/2012
			'groups' => array(
				'groupType' => '/groups/v1/grouptypes',
		...
		/**
		 * fetch group types
		 * added by: Bethany Clark on 02/17/2012
		 *
		 * Intended for a group search, this uses the simple grouptypes uri and defaults to searching
		 * for those grouptyess that are "issearchable=true".
		 */
		public function getGroupTypes($append = "/search.json?issearchable-true"){
			$url = $this->settings->baseUrl . $this->paths['groups']['groupType'] . $append;
			return $this->fetchGetJson($url);
		}
		 ...
		 * added by: Bethany Clark on 02/17/2012
		 *
		 * Intended for a group search, this uses the simple group uri and defaults to searching
		 * for those groups that are "issearchable=true".
		 */
		public function getGroupIndividual($append = "/search.json?issearchable-true"){
		        $url = $this->settings->baseUrl . $this->paths['groups']['group'] . $append;
                        return $this->fetchGetJson($url);
		}
